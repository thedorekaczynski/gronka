...
describe('gif optimizer utilities', function describeGifOptimizerUtilities() {
  describe('isGifFile', function describeIsGifFile() {
    test('detects GIF from extension', function testDetectsGIFFromExtension() {
      assert.strictEqual(isGifFile('file.gif', ''), true);
      assert.strictEqual(isGifFile('file.GIF', ''), true);
      assert.strictEqual(isGifFile('path/to/file.gif', ''), true);
    });

    test('detects GIF from content type', function testDetectsGIFFromContentType() {
    ...
    test('returns false for non-GIF files', function testReturnsFalseForNonGIFFiles() {
    ...
    test('returns true if either extension or content type matches', function testReturnsTrueIfEitherExtensionOr() {
  ...
  describe('extractHashFromCdnUrl', function describeExtractHashFromCdnUrl() {
    test('extracts hash from cdn.gronka.dev URL', function testExtractsHashFromCdnGronkaDev() {
    ...
    test('extracts hash from other gronka.dev subdomain', function testExtractsHashFromOtherGronkaDev() {
      const url = 'https://subdomain.gronka.dev/gifs/abc123def456.gif';
      const hash = extractHashFromCdnUrl(url);
      assert.strictEqual(hash, 'abc123def456');
    });

    test('extracts hash from cdn.gronka.dev video URL', function testExtractsHashFromCdnGronkaDev() {
      const url = 'https://cdn.gronka.dev/videos/abc123def456.mp4';
      const hash = extractHashFromCdnUrl(url);
      assert.strictEqual(hash, 'abc123def456');
    });

    test('handles URLs with query parameters', function testHandlesURLsWithQueryParameters() {
      const url = 'https://cdn.gronka.dev/gifs/abc123.gif?version=1&cache=true';
      const hash = extractHashFromCdnUrl(url);
      assert.strictEqual(hash, 'abc123');
    });

    test('handles URLs with fragments', function testHandlesURLsWithFragments() {
      const url = 'https://cdn.gronka.dev/gifs/abc123.gif#section';
      const hash = extractHashFromCdnUrl(url);
      assert.strictEqual(hash, 'abc123');
    });

    test('returns null for non-cdn domains', function testReturnsNullForNonCdnDomains() {
    ...
    test('returns null for invalid path pattern', function testReturnsNullForInvalidPathPattern() {
    ...
    test('returns null for invalid URL format', function testReturnsNullForInvalidURLFormat() {
      assert.strictEqual(extractHashFromCdnUrl('not a url'), null);
      assert.strictEqual(extractHashFromCdnUrl(''), null);
    });

    test('handles case-insensitive hash', function testHandlesCaseInsensitiveHash() {
  ...
  describe('calculateSizeReduction', function describeCalculateSizeReduction() {
    test('calculates correct reduction percentage', function testCalculatesCorrectReductionPercentage() {
    ...
    test('returns negative for file growth', function testReturnsNegativeForFileGrowth() {
      assert.strictEqual(calculateSizeReduction(1000, 1100), -10);
      assert.strictEqual(calculateSizeReduction(1000, 1500), -50);
    });

    test('handles zero original size', function testHandlesZeroOriginalSize() {
      assert.strictEqual(calculateSizeReduction(0, 100), 0);
      assert.strictEqual(calculateSizeReduction(0, 0), 0);
    });

    test('rounds to nearest integer', function testRoundsToNearestInteger() {
  ...
  describe('formatSizeMb', function describeFormatSizeMb() {
    test('formats bytes to MB', function testFormatsBytesToMB() {
